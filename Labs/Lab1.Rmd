...
*Question 1:* Create a scatter plot using price on the y-axis and month on the x-axis.
...
ggplot(data = pumpkins, aes(x = day, y = price))+
  ...
  labs(x = "Day of Year",
       y = "Price ($)",
       title = "Price of Pumpkins Depending on the Day of Year")
```
...
new_pumpkins %>% 
  ...
  geom_point(size = 1.6, color = "orange")+
  labs(y = "Price per bushel")
```

***Question 3:*** **Is this a useful plot? Does anything about it surprise you?**

This is a semi useful plot. You can get an idea of what the min and max value you'd expect to pay for pumpkins on a given day but it really isn't clear about how much pumpkin you're getting because the packages are different sizes. Also many people know the date but they don't actually know the day of the year.
...
summary_pumpkins <- new_pumpkins %>% 
  group_by(month) %>% 
  summarize(avg_price = mean(price))

ggplot(data = summary_pumpkins, aes(x = month, y = avg_price))+
  geom_col(fill = "#D16002")+
  labs(x = "Month",
       y = "Average Price of Pumpkins ($/Bushel)",
       title = "Monthly Average Price of Pumpkin Bushels")
```
...
There are 10 cities represented in the dataset.
...
The highest correlation is between the package and the price. This makes sense because as the size of the package increases, the price should increase as well.
...
This data doesn't follow a linear relationship, so using a linear regression to fit this data isn't the best model to useß.
